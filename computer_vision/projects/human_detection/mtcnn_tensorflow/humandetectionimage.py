...
def main():
    ...
    with tf.Session() as sess:
        ...
        image_file_name = "./d4bed9d4d2201ac0712732.jpg"
        image = cv2.imread(image_file_name)
        rectangles = detect_human(image, minsize,
                                pnet_fun, rnet_fun, onet_fun,
                                threshold, factor)
        for i in range(rectangles.shape[0]):
            cv2.rectangle(image, (int(rectangles[i][0]), int(rectangles[i][1])),
                          (int(rectangles[i][2]), int(rectangles[i][3])), [255, 0, 0], 2)
            cv2.putText(image, '%.2f' % rectangles[i][4], (int(rectangles[i][0]), int(rectangles[i][1])),
