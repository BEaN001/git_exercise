...
def read_and_decode_cls(tfrecords_file, batch_size):
    ...
    image.set_shape([144 * 48 * 3])
    image = tf.reshape(image, [144, 48, 3])
...
START_LR = 0.0001
iteration = 500
tfrecord_files = ['/Users/binyu/Documents/git_exercise/computer_vision/projects/human_detection/objectdetection_data/native_48/ONet_data_for_cls.tfrecords',
                  '/Users/binyu/Documents/git_exercise/computer_vision/projects/human_detection/objectdetection_data/native_48/ONet_data_for_bbx.tfrecords']
...
data_images = tf.placeholder(tf.float32, shape=[batch_size, 144, 48, 3])  # input x1
...
def O_Net(inputs):
    with tf.variable_scope('O_Net'):
        with slim.arg_scope([slim.conv2d],
            ...
            net = slim.max_pool2d(net, kernel_size=[3, 3], stride=2, scope="opool1", padding='SAME')  # 70, 23, 32
            net = slim.conv2d(net, num_outputs=64, kernel_size=[5, 3], stride=1, scope="oconv2")  # 66, 21, 64
            net = slim.max_pool2d(net, kernel_size=[3, 3], stride=2, scope="opool2")  # 32, 10, 64
            net = slim.conv2d(net, num_outputs=64, kernel_size=[5, 3], stride=1, scope="oconv3")  # 28, 8, 64
            net = slim.max_pool2d(net, kernel_size=[2, 2], stride=2, scope="opool3", padding='SAME')  # 14, 4, 64

            net = slim.conv2d(net, num_outputs=64, kernel_size=[5, 3], stride=1, scope="oconv4")  # 10, 2, 64
            net = slim.max_pool2d(net, kernel_size=[2, 2], stride=2, scope="opool4", padding='SAME')  # 5, 1, 64

            net = slim.conv2d(net, num_outputs=128, kernel_size=[3, 1], stride=1, scope="oconv5")  # 3, 1, 128

            fc_flatten = slim.flatten(net)
            fc1 = slim.fully_connected(fc_flatten, num_outputs=256, scope="ofc1", activation_fn=prelu)
            fc2_1 = slim.fully_connected(fc1, num_outputs=2, scope="ofc2_1", activation_fn=None)
            fc2_2 = slim.fully_connected(fc1, num_outputs=4, scope="ofc2_2", activation_fn=None)

            return (fc2_1, fc2_2)


out_put = O_Net(data_images)
...
with tf.Session() as sess:
    ...
    coord.join(threads)
    saver.save(sess, '/Users/binyu/Documents/git_exercise/computer_vision/projects/human_detection/objectdetection_data/native_48/model/trained_model.ckpt')
