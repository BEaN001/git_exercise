...
from computer_vision.projects.hand_pose_estimation.cpm_pytorch.src.util import *
...
class CMUHand(Dataset):
    ...
    def genLabelMap(self, label, label_size, joints, ratio_x, ratio_y):
        ...
        """
        # initialize
        label_maps = np.zeros((joints, int(label_size), int(label_size)))
        background = np.zeros((int(label_size), int(label_size)))
...
if __name__ == "__main__":
    data_dir = '/Users/binyu/Documents/git_exercise/computer_vision/algo_datas/CMUHand/algodata/'
    label_dir = '/Users/binyu/Documents/git_exercise/computer_vision/algo_datas/CMUHand/label/'
    data = CMUHand(data_dir=data_dir, label_dir=label_dir)

    img, label, center, name = data[2]
    print('dataset info ...')
    ...
    imageio.imwrite('img/cmu_label.jpg', out_labels)
