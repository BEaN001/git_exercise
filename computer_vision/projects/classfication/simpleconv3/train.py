from computer_vision.projects.classfication.simpleconv3.dataset import ImageData
    ...
    with tf.Session() as sess:
        ...
        for i in range(steps):
            _, cross_entropy_, accuracy_, batch_images_, batch_labels_, loss_summary_, acc_summary_, image_summary_ = sess.run(
                [train_step, cross_entropy, accuracy, batch_images, batch_labels, loss_summary, acc_summary,
                 image_summary])
            if i % in_steps == 0:
